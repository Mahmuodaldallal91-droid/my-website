...
// Default: open Reviews section on first load
  ...
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      // Keep navigation inside the page and avoid any unintended "jump to top" behaviors
      if (e) {
      ...
      // Scroll to the START of the section reliably (Safari + Android).
      // Use a delayed double-pass to account for accordion height transitions.
      const doScrollToStart = (behavior) => {
        try {
          const offset = (typeof getStickyOffset === "function") ? getStickyOffset() : 0;
          const y = (el.getBoundingClientRect().top + (window.pageYOffset || 0)) - offset - 10;
          const top = Math.max(0, Math.round(Number.isFinite(y) ? y : 0));
          try {
      ...
      // First pass (smooth)
      requestAnimationFrame(() => {
        doScrollToStart("smooth");
      });

      // Second correction after layout settles (fix iOS/Safari jump & wrong anchoring)
      setTimeout(() => {
        doScrollToStart("auto");
      }, 320);
