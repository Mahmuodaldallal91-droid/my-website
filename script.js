...
(() => {
  ...
  if (reviewForm && rfName && rfComment && reviewsList) {
    reviewForm.addEventListener("submit", (e) => {
      ...
      card.innerHTML = `
        <div class="reviewTop">
          <div class="reviewName"></div>
          <div class="reviewStars" aria-label="5 stars">★★★★★</div>
        </div>
        <div class="reviewBody"></div>
      `;
      card.querySelector(".reviewName").textContent = name;
      card.querySelector(".reviewBody").textContent = comment;
      ...
      if (rfMsg) {
        const lang = document.documentElement.lang || "ar";
        rfMsg.textContent = (dict.commentThanks && dict.commentThanks[lang]) ? dict.commentThanks[lang] : "تم استلام تعليقك. شكرًا لك!";
        setTimeout(() => { rfMsg.textContent = ""; }, 2200);
      }
    });
  }
