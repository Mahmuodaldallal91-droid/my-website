...
alreadyRated: { ar: "تم استلام تقييمك مسبقًا.", en: "You already rated.", he: "כבר דירגת." },
  ...
  function renderReviews(lang, reset){
    ...
    const nextCount = Math.min(all.length, reviewsShown === 0 ? INITIAL_REVIEWS : (reviewsShown + REVIEWS_PAGE));
    ...
    if(moreBtn){
      ...
      moreBtn.onclick = () => {
        ...
        // After rendering new cards, gently scroll to keep the "load more" button centered
        requestAnimationFrame(()=>{
          requestAnimationFrame(()=>{
            const stillVisible = !moreBtn.hidden;
            const target = stillVisible ? moreBtn : (document.querySelector("#reviewsList .reviewCard:last-child") || moreBtn);
            try{
              scrollElementToCenter(target, 900);
              setTimeout(()=>scrollElementToCenter(target, 600), 240);
            }catch(_e){}
          });
        });
      };
...
(() => {
  ...
  function smoothScrollToY(targetY, durationMs){
    ...
    const destY = Math.max(0, Math.min(maxY, targetY));
    const delta = destY - startY;
    if(Math.abs(delta) < 1){ return; }

    const startT = performance.now();
    const dur = Math.max(250, Number(durationMs)||900);
  ...
  function scrollElementToCenter(el, durationMs){
    ...
    const targetY = (r.top + (window.pageYOffset||0)) - ((window.innerHeight/2) - (r.height/2)) - (offset/2);
    smoothScrollToY(targetY, durationMs);
  }

  function scrollSectionToCenter(sectionEl, durationMs){
    if(!sectionEl) return;
    // Prefer centering the first heading in the section (looks more "professional")
    const anchor = sectionEl.querySelector("h2, h3, .panelTitle, .sectionTitle") || sectionEl;
    scrollElementToCenter(anchor, durationMs);
  }
...
// Default: open Reviews section on first load
  ...
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // Smooth center scroll to the section (slow + professional)
      try{
        scrollSectionToCenter(el, 1000);
        // Safari/iOS sometimes reflows after opening panels; correct once more.
        setTimeout(()=>scrollSectionToCenter(el, 700), 260);
      }catch(_e){}
