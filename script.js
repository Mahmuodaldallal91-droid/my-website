...
// ✅ تقييد عدد الطلبات لكل جهاز/متصفح (3 طلبات فقط)
const ORDER_LIMIT = 3;
const ORDER_COUNT_KEY = "order_submit_count_v1";
function getOrderCount(){
  return parseInt(localStorage.getItem(ORDER_COUNT_KEY) || "0", 10);
}
function setOrderCount(n){
  localStorage.setItem(ORDER_COUNT_KEY, String(n));
}
function lockOrderButtonIfNeeded(){
  if(!submitOrder) return;
  if(getOrderCount() >= ORDER_LIMIT){
    // بدون إشعار للمستخدم: إخفاء الزر نهائيًا
...
// ✅ طبّق التقييد فور التحميل
setTimeout(lockOrderButtonIfNeeded, 0);
...
function pauseProgress(){
  ...
  const pct = Math.max(0, Math.min(100, (w / pw) * 100));
  progressBar.style.transition = "none";
...
// إيقاف/تشغيل تلقائي عند المرور (يعطي إحساس فخم)
...
if(carouselEl){
  ...
  carouselEl.addEventListener("touchend", (e) => {
    ...
    // تجاهل السحب العمودي
    if(Math.abs(dy) > Math.abs(dx)) { startAuto(); return; }
...
function updateLoadMoreBtnUI(){
  ...
  if(loadMoreBtnText) loadMoreBtnText.textContent = remaining > 0 ? "عرض المزيد من آراء العملاء" : "تم عرض جميع آراء العملاء";;
...
function showMoreReviews(){
  ...
  const prevShown = shownReviewsCount;
  ...
  setTimeout(() => {
    ...
    // Animate only the newly shown items
    const items = reviewsList ? Array.from(reviewsList.querySelectorAll(".reviewItem")) : [];
    items.forEach((el, i) => {
      if(i >= prevShown) {
        el.classList.add("is-new");
        // remove after animation so it won't replay on other renders
...
submitOrder.addEventListener("click", () => {
  ...
  // ✅ منع الإرسال إذا وصل الحد (بدون تنبيه)
  if(getOrderCount() >= ORDER_LIMIT){
    lockOrderButtonIfNeeded();
    return;
  }
  ...
  // ✅ إرسال نسخة من الطلب إلى الإيميل (AJAX) بدون ما يطلع المستخدم من الصفحة
(async () => {
  try {
    const endpoint = "https://formsubmit.co/ajax/mahmuod.aldallal91@icloud.com";

    const fd = new FormData();
    fd.append("_subject", `طلب جديد - ${name}`);
    fd.append("_template", "table");
    fd.append("_captcha", "false");

    // نفس أسماء الحقول بالعربي لظهورها مرتبة في الإيميل
    fd.append("الاسم", name);
    fd.append("الهاتف", phone);
    fd.append("الدولة", country || "-");
    fd.append("المدينة", city || "-");
    fd.append("العنوان", address);
    fd.append("الكمية", qty);
    fd.append("الدفع", (pay === "cash") ? "عند الاستلام" : "بطاقة");
    fd.append("ملاحظات", notes || "-");
    fd.append("وقت الإرسال", new Date().toISOString());
    ...
    // ✅ زِد العداد فقط إذا تم الإرسال بنجاح (حتى ما نقفل الزر بدون ما يوصل الإيميل)
    if (resp.ok) {
      const nextCount = getOrderCount() + 1;
      setOrderCount(nextCount);
      if (nextCount >= ORDER_LIMIT) lockOrderButtonIfNeeded();
    }
  } catch (e) {
    // لا نوقف تجربة المستخدم إذا فشل الإرسال لأي سبب
    console.warn("Email submit failed", e);
  }
})();
