...
alreadyRated: { ar: "تم استلام تقييمك مسبقًا.", en: "You already rated.", he: "כבר דירגת." },
  ...
  function updateReviewSummary(all, lang){
    ...
    const avgLabel = (dict.reviewsAvgLabel && dict.reviewsAvgLabel[lang]) || "";
    rsAvg.textContent = avgLabel ? `${avgLabel}: ${avgFixed}/5` : `${avgFixed}/5`;
  ...
  function renderReviews(lang, reset){
    ...
    if(moreBtn){
      ...
      moreBtn.onclick = () => {
        ...
        // keep the "load more" button aligned at the top (prevents iOS jump)
        setTimeout(() => scrollToButtonTop(moreBtn), 80);
        setTimeout(() => scrollToButtonTop(moreBtn), 260);
      };
    }
  }
...
(() => {
  ...
  function scrollToButtonTop(btn){
    ...
    const top = btn.getBoundingClientRect().top + window.scrollY - off - 8;
    window.scrollTo({ top, behavior: "smooth" });
    // avoid iOS focus jump
  ...
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // scroll so the pressed button starts at the top (under sticky lang bar)
      scrollToButtonTop(btn);
      // second alignment pass (Safari/iOS layout shifts)
      setTimeout(() => scrollToButtonTop(btn), 260);
    });
  });
